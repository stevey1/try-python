class Graph():
    ...
    def depth_first_shortest_path(self, curr, start):
        ...
        for vertex in range(len(self.matrix[curr])):
            if self.matrix[curr][vertex] >0 and not vertex==start:
                if self.distances[vertex]>0:
                    self.distances[vertex]= min(self.distances[vertex], self.distances[curr]+self.matrix[curr][vertex])
                else:
                    self.distances[vertex]= self.distances[curr]+self.matrix[curr][vertex]
    ...
    def breath_first_shortest_path(self, curr):
        ...
        for vertex in range(len(self.matrix)):
            if vertex not in self.visited and self.matrix[vertex][curr]>0:
                if self.distances[vertex]>0:
                    self.distances[vertex]= min(self.distances[vertex], self.distances[curr]+self.matrix[vertex][curr])
                else:
                    self.distances[vertex]= self.distances[curr]+self.matrix[vertex][curr]
        for vertex in range(len(self.matrix)):
            if vertex not in self.visited and self.matrix[vertex][curr]>0:
